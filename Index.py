...
def handle_transaction():
    ...
    if transaction_type == 'issue':
        if book.quantity <= 0:
            return jsonify({"message": "Book not available for issue."}), 400
        book.quantity -= 1
        db.session.add(Transaction(book_id=book_id, member_id=member_id, transaction_type='issue'))
        db.session.commit()
        return jsonify({"message": "Book issued successfully."})
    elif transaction_type == 'return':
        book.quantity += 1
        rent_fee = data.get('rent_fee', 0)
        if member.outstanding_debt + rent_fee > 500:
            return jsonify({"message": "Member's outstanding debt exceeds Rs. 500. Cannot issue book."}), 400
        member.outstanding_debt += rent_fee
        db.session.add(Transaction(book_id=book_id, member_id=member_id, transaction_type='return', rent_fee=rent_fee))
        db.session.commit()
